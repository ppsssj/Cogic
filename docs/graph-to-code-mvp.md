...
이 문서는 graph-to-code 아이디어를 현재 Cogic 코드베이스에 맞는 구체적인 MVP 계획으로 정리한 문서다.
...
Cogic는 개발자가 그래프 UI에서 구조를 설계하고, 그 구조를 안전한 코드 스캐폴드 패치로 미리보기한 뒤 적용할 수 있게 해주는 도구다.
...
5. `Generate`를 누른다.
6. Cogic가 다음 파일들에 대한 패치 미리보기를 보여준다.
   - `user-service.ts`
   - `user-repository.ts`
   - 필요하다면 export 업데이트
7. 사용자가 패치를 적용한다.
...
4. `Generate`를 누른다.
5. Cogic가 선택한 파일에 새 선언을 삽입하는 패치를 미리보기로 보여준다.
...
4. Cogic가 선언 헤더와 필요한 import를 생성한다.
...
이 기능은 매력적이지만, Cogic를 처음 도입하게 만드는 1순위 이유는 아닐 가능성이 크다.
