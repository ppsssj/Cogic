# CodeGraph Product Notes

Last updated: 2026-03-23

이 문서는 CodeGraph의 제품 방향, 차별화 포인트, 확장 기능 아이디어를 누적 기록하는 작업 메모다.
...
- 따라서 CodeGraph가 `코드를 설명해주는 또 하나의 AI 도구`로 포지셔닝되면 설득력이 약하다.
- CodeGraph의 의미는 `결정적인 구조 정보`, `탐색 가능한 관계`, `변경 영향도`, `경로 추적` 쪽에서 더 커진다.

### 냉정한 판단

- CodeGraph는 여전히 가치가 있다.
...
- CodeGraph를 `읽는 도구`에서 `설계하고 시작 코드를 만드는 도구`로 확장할 수 있다.
